...
public class PolygonBeamStaticPulseSimple : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (beam) //Updates the beam
        {
            ...
            if (beamCollides && Physics.Raycast(transform.position, transform.forward, out hit, beamLength)) //Checks for collision
            {
